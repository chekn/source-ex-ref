...
public abstract class AbstractMessageConverterMethodProcessor extends AbstractMessageConverterMethodArgumentResolver
	...
	protected <T> void writeWithMessageConverters(T returnValue, MethodParameter returnType,
		...
		String specialCT=responseBody.contentType();
		if(specialCT.equals("")) {
			HttpServletRequest servletRequest = inputMessage.getServletRequest();
			List<MediaType> requestedMediaTypes = getAcceptableMediaTypes(servletRequest);
			List<MediaType> producibleMediaTypes = getProducibleMediaTypes(servletRequest, returnValueClass);
			logger.info("returnValueClass [" + returnValueClass.getName() + "]");
			logger.info("requestedMediaTypes [" + requestedMediaTypes + "]");
			logger.info("producibleMediaTypes [" + producibleMediaTypes + "]");
			...
			selectedMediaType=MediaType.valueOf(specialCT);
		}

		logger.info("Written [" + returnValue + "] as \"" + selectedMediaType + "\" using [" + "]");
	 ...
	 * 为什么Spring在这里搞这么个烂东西，已使用它的ResponseBody 就下载
	 */
	private void addContentDispositionHeader(ServletServerHttpRequest request,
		...
		System.out.println("路径扩展名:"+extInPathParams+"_");
		
		for(String str:safeExtensions)
			System.out.println("安全的扩展名:"+str);
